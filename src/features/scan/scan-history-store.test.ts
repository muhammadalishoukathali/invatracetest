...
import {
  ...
  updateScanHistorySubmission,
} from './scan-history-store'
...
describe('scan history storage', () => {
  ...
  it('tracks queued and submitted reports against their capture', () => {
    const storage = new MemoryStorage()
    saveScanHistoryRecord(record('submitted', '2026-09-01T09:00:00.000Z'), storage)

    updateScanHistorySubmission('submitted', { status: 'queued' }, storage)
    expect(listScanHistory(storage)[0]?.submission).toEqual({ status: 'queued' })

    updateScanHistorySubmission('submitted', { status: 'submitted', reportId: 'report-1' }, storage)
    expect(listScanHistory(storage)[0]?.submission).toEqual({
      status: 'submitted', reportId: 'report-1',
    })
  })

  it('clears identity-bound scan records on sign-out', () => {
    const storage = new MemoryStorage()
    saveScanHistoryRecord(record('private', '2026-09-01T09:00:00.000Z'), storage)

    clearScanHistory(storage)
