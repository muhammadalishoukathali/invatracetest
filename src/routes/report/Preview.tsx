...
import { submitReport } from '@/lib/report-queue'
...
export function Preview() {
  ...
  const canSubmit = !!draft.location && !!imageBlob && draft.consentAccurate && draft.consentNoPII
  ...
  return (
    <div style={{ padding: 16, maxWidth: 520, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <Card>
        <Row icon="Leaf"
             label="Species"
             value={draft.speciesId ?? 'Unknown — flagged for expert review'} />
        <Divider />
        ...
        <Row icon="MapPin"
             ...
             value={draft.location
               ...
               : '—'}
             sub={draft.locationAccuracyM != null ? `±${draft.locationAccuracyM} m GPS` : 'Manual entry'}
             mono />
        <Divider />
        <Row icon="Grid3x3"
             label="Extent"
